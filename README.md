...
酷Q插件启动的时候会运行JPRE的Jar包, 然后JPRE加载Java插件, 
酷Q插件接受到酷Q事件后, 会通过易语言Java支持库传递给JPRE, 然后JPRE传递给Java插件.
`
...
4. **一句注册** 一句代码即可自动注册整个类的事件检测器 (并且不需要预先在插件信息中定义)
5. **允许重复** 一个插件可以注册多个事件监视器来进行不同种类工作

#### 多样配置方法支持
运行环境已经为你开发好了一系列的读取/写入方法，插件直接调用即可
目前支持的配置方法
1. Yaml (<a herf="http://mvnrepository.com/artifact/org.yaml/snakeyaml">SnakeYAML</a>)
2. Json (<a herf="https://github.com/google/gson">Gson</a>)
...
- 在使用过程中遇到任何问题, 或是有好的建议, 请提交 issue, 

#### 联系我们
- Java 运行环境(JPRE)(主要开发): Him188  QQ1040400290
